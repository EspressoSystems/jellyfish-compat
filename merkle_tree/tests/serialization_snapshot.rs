...
fn test_fee_merkle_tree_serialization() {
    ...
    for tx_id in 1u64..=5u64 {
        if let jf_merkle_tree_compat::LookupResult::Ok(elem, proof) = tree.universal_lookup(tx_id) {
...
fn test_block_merkle_tree_serialization() {
    ...
    for i in 0..tree.num_leaves() {
        if let jf_merkle_tree_compat::LookupResult::Ok(elem, proof) = tree.lookup(i) {
...
fn test_rewards_tree_serialization() {
    ...
    for &(account, _reward) in &rewards {
        if let jf_merkle_tree_compat::LookupResult::Ok(elem, proof) = tree.universal_lookup(account)
        {
...
fn test_rewards_tree_non_membership_proofs() {
    ...
    for &account in &non_member_accounts {
        if let jf_merkle_tree_compat::LookupResult::NotFound(proof) = tree.universal_lookup(account)
        {
...
fn test_fee_tree_non_membership_proofs() {
    ...
    for &tx_id in &non_member_txs {
        if let jf_merkle_tree_compat::LookupResult::NotFound(proof) = tree.universal_lookup(tx_id) {
...
fn test_single_element_tree_serialization() {
    ...
    if let jf_merkle_tree_compat::LookupResult::Ok(elem, proof) = tree.lookup(0u64) {
