...
use jf_merkle_tree_compat::{
    ...
    MerkleCommitment, MerkleTreeCommitment, MerkleTreeScheme,
};
#[cfg(feature = "gpu-vid")]
    ...
    AdvzInternal<E, H, T>: MaybeGPU<E>,
{
    ...
    fn verify_share(
        ...
        // This is hacky because we changed the Merkle tree interface but not the VID
        // related structs.
        let proof_height = share
            ...
            .len()
            .checked_sub(1)
            .ok_or_else(|| VidError::Argument("Empty share proof".to_string()))?;
        let commitment = MerkleTreeCommitment::new(common.all_evals_digest, proof_height, 0);
